...
} from '@shared/painting/paintingGenerateError'
...
export function translatePaintingGenerateError(error: Error): string {
  ...
  const keyMap: Record<Exclude<PaintingGenerateErrorCode, 'REMOTE_ERROR'>, string> = {
    ...
    // fork 对齐：paintingImageService 既有码，复用 fork 已有的 provider_disabled 文案键。
    PROVIDER_NOT_ENABLED: 'error.provider_disabled',
    ...
    // fork 对齐：paintingImageService 既有码，与 GENERATE_FAILED 同文案键。
    GENERATION_FAILED: 'paintings.generate_failed',
    ...
    UNSUPPORTED_VENDOR: 'paintings.unsupported_vendor'
  }
