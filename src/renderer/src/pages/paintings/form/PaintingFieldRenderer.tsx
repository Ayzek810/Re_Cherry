...
export function PaintingFieldRenderer({ item, painting, onChange, onGenerateRandomSeed }: PaintingFieldRendererProps) {
  ...
  switch (item.type) {
          ...
          {options.map((option) => {
            ...
            return (
              <label key={optionValue} htmlFor={inputId} className="flex cursor-pointer items-center gap-2 text-sm">
                <Radio id={inputId} value={optionValue} />
    ...
    case 'slider': {
      ...
      // nowhere to move and renders its thumb flush to the rail edge,
      ...
      return (
        <div className="flex items-center gap-2">
          ...
          {fieldKey.toLowerCase().includes('seed') && onGenerateRandomSeed ? (
            <Button size="small" onClick={() => onGenerateRandomSeed(fieldKey)}>
              <RotateCcw size={14} />
            </Button>
          ) : null}
        </div>
    ...
    case 'switch': {
      return (
        <div className="flex items-center">
          <Switch checked={Boolean(currentValue)} onChange={(checked) => onChange({ [fieldKey]: checked })} />
        </div>
      )
    }
