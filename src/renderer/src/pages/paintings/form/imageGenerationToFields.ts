 ...
 *      `@shared/lightLlm/imageGenerationCatalog` 引（fork 不搬 provider-registry 包）；
 *   2. `initialValue` 的取值来源同 V2（`spec.default`），无 registry 查询。
 * `KEY_LABELS` 保持对 `CanonicalParamKey` 穷举（表加键不加标签即编译错）。
 */
import type {
  CanonicalParamKey,
  ImageGenerationMode,
 ...
 * canonical key MUST have a label, so adding a key to `CANONICAL_PARAM_KEYS`
 * without a label here is a compile error (rather than a silent raw-key render).
 * Adding a new canonical control is a one-row addition here + a registry data
 * entry on the relevant models — no schema change, no per-key handler.
 */
