...
// ③ 类型缝：UniqueModelId ← @shared 对口面；isUniqueModelId ← 本页 cliConfig/values；
//   Model ← providerView 投影；claudeModelMode 面（detailed 模式）随①移除。
// ④ fork 面注：fork 的 cliConfigConnectionMatchesProvider 第 3 参为投影 Provider（同形）。
...
export function useConfigDraftController({
  ...
  const computeIsDirty = useCallback((nextDraft: ConfigDraft) => {
  ...
  const resolveManagedOptions = useCallback(
    (config: Record<string, unknown>, modelId: UniqueModelId | undefined) =>
      resolveManagedDraftOptions(cliTool, provider.id, config, modelId, gateway ? models : undefined),
  ...
  // fork 缝①（续）：V2 的 managedSubmitModelId 同值（detailed 臂删除后为 modelId 直读）。
  const managedSubmitModelId = resolveManagedOptions(draft.config, draft.modelId).cliConfigModelId
  ...
  const handleModelSelect = useCallback(
    (nextModelId: UniqueModelId | undefined) => {
      ...
      if (nextModelId) {
        loadManagedDraft(nextModelId, current.config, current.files, {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (current.mode === 'foreign') {
        ...
        const cliConfigModelId = current.modelId
        const submitDraft = cliConfigModelId
          ? await createManagedDraft(current.modelId, sanitizedConfig, current.files, {
              cliConfigModelId,
              writePrimaryModel: true
            })
          : null
