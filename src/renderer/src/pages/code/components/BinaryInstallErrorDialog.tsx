...
export const BinaryInstallFailureRow: FC<{ error: string; onShowError: () => void }> = ({ error, onShowError }) => {
  ...
  return (
    <button
      ...
      onClick={onShowError}
      title={error}
      className="mt-3 flex w-full min-w-0 items-center gap-1.5 rounded-lg border border-error/35 bg-error/15 px-2.5 py-1.5 text-left text-[11px] text-error transition-colors hover:bg-error/25">
...
export const BinaryInstallErrorDialog: FC<{
  ...
  const copyError = () => {
    ...
    navigator.clipboard.writeText(lastError.current.message).then(
      ...
      () => {
        if (requestId === copyRequestId.current) window.toast.error(t('common.copy_failed'))
      }
    )
  }
