 ...
 * - 技能文件列举/读取的主进程 IPC 尚未接线 → 文件树与内容保持为空，
 *   详情页如实显示空状态（接线批次恢复上游 promise 逻辑）；
 * - 安装/卸载等动作数据源改为 redux 切片（见 hooks/useSkills.ts）。
 */
...
const SkillsSettings: FC = () => {
  ...
  // Fork: 无 IPC 加载即完成，loadingContent 恒为 false（上游 setter 随 readSkillFile 移除）。
  ...
  // Fork: 技能文件读取的主进程 IPC 尚未接线——内容保持为空。
  useEffect(() => {
  ...
  const handleInstall = useCallback(
    async (result: SkillSearchResult) => {
      const { skill, error } = await install(result)
      if (skill) {
            ...
            {selectedSkill ? (
              loadingContent ? (
                ...
                isMarkdownFile(selectedFile) ? (
                  <MarkdownContainer>
                    <MarkdownPre key={selectedFile}>{fileContent}</MarkdownPre>
                  </MarkdownContainer>
                ) : (
                  <CodeViewerContainer>
