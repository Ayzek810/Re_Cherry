...
const McpSettings: React.FC = () => {
  ...
  const fetchServerLogs = async () => {
    try {
      const history = await mcpApi.getServerLogs(server)
      setLogs(history)
    } catch (error) {
  ...
  useEffect(() => {
    const unsubscribe = mcpApi.onServerLog((log) => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (server.isActive) {
        try {
          await mcpApi.restartServer(mcpServer)
  ...
  const onToggleActive = async (active: boolean) => {
    ...
    try {
      if (active) {
        const localTools = await mcpApi.listTools(serverForUpdate)
        setTools(localTools)

        const localPrompts = await mcpApi.listPrompts(serverForUpdate)
        setPrompts(localPrompts)

        const localResources = await mcpApi.listResources(serverForUpdate)
        setResources(localResources)

        const version = await mcpApi.getServerVersion(serverForUpdate)
        setServerVersion(version)
      } else {
  ...
  const handleToggleTool = useCallback(
    async (tool: MCPTool, enabled: boolean) => {
      ...
      // await mcpApi.updateServer(updatedServer)
