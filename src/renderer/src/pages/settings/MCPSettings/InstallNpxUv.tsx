...
import { installBunBinary, installUVBinary, isBinaryExist, mcpApi } from '@renderer/services/mcpApi'
...
const InstallNpxUv: FC<Props> = ({ mini = false }) => {
  ...
  const checkBinaries = useCallback(async () => {
    const uvExists = await isBinaryExist('uv')
    const bunExists = await isBinaryExist('bun')
    const { uvPath, bunPath, dir } = await mcpApi.getInstallInfo()
