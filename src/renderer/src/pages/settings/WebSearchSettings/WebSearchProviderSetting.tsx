...
const WebSearchProviderSetting: FC<Props> = ({ providerId }) => {
  ...
  const getWebSearchProviderLogo = (id: WebSearchProviderId) => {
    switch (id) {
      case 'zhipu':
        return ZhipuLogo
      case 'tavily':
  ...
  const handleCheckSearch = async () => {
    ...
    try {
      setApiChecking(true)
      const valid = await checkSearch(provider.id)
      setApiValid(valid)
      window.toast[valid ? 'success' : 'error']({
        timeout: valid ? 2000 : 8000,
        title: valid ? t('settings.tool.websearch.check_success') : t('settings.tool.websearch.check_failed')
      })
    } catch (error) {
      logger.warn('Check search failed', error as Error)
      setApiValid(false)
    ...
    <>
      ...
      {isLocalProvider && (
        <>
          ...
          <Tooltip title={t('settings.tool.websearch.local_provider.hint')} placement="right">
            <Button type="primary" disabled icon={<ExportOutlined />}>
