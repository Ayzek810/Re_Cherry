...
export function consolidateReferencesByUrl(
  ...
  const sourceGroups = references.reduce((groups, reference) => {
    const originalResult = urlToOriginalResult.get(reference.source)
    if (!originalResult) return groups

    const existing = groups.get(reference.source)
    if (existing) {
      // 如果已存在该URL的分组，直接添加内容
      existing.contents.push(reference.content)
    } else {
      // 创建新的分组
      groups.set(reference.source, {
        originalResult,
...
export function selectReferences(
  ...
  // 按source分组references，每组内按原顺序保持（已按分数排序）
  const groupsByUrl = new Map<string, KnowledgeReference[]>()
  references.forEach((ref) => {
    if (!groupsByUrl.has(ref.source)) {
      groupsByUrl.set(ref.source, [])
    }
    groupsByUrl.get(ref.source)!.push(ref)
  })
