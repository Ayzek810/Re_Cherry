 ...
 *
 * `host`/`port` default to the gateway's configured defaults so the integration
 * tests can call `buildApp()` with no arguments; `server.ts` passes the live
 * configured values. They populate the OpenAPI `servers` URL so Scalar renders
 * copyable absolute curl examples (e.g. `curl http://127.0.0.1:23333/health`)
 * instead of relative paths.
 *
 * Exported for the runtime server (`server.ts`).
 */
