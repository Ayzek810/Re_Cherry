...
// Mock ConfigManager（fork 缝：V2 经 @test-mocks/main/application mock
// PreferenceService；fork 的 auth 直接读 ConfigManager 单例）。
const { mockGetApiGatewayApiKey } = vi.hoisted(() => ({
  mockGetApiGatewayApiKey: vi.fn()
}))

vi.mock('@main/services/ConfigManager', () => ({
  configManager: {
    getApiGatewayApiKey: mockGetApiGatewayApiKey
  }
}))
...
describe('authorizeApiRequest', () => {
  ...
  describe('API Key authentication (priority)', () => {
    ...
    beforeEach(() => {
      mockGetApiGatewayApiKey.mockReturnValue(validApiKey)
    })
