...
type GatewayErrorContext = Parameters<ErrorHandler>[0]
...
/**
 * Anthropic-dialect error handler (`/v1/messages`). Shapes built-in failures into
 * the Anthropic envelope; delegates provider/runtime errors to
 * `transformAnthropicError`. fork 缝：V2 的 `DataApiError` 分支随 DataApi 数据层
 * 一并裁掉（fork 无该子系统）。
 */
 ...
 * failures into the OpenAI envelope; delegates provider/runtime errors to
 * `transformOpenAiError`. fork 缝：`DataApiError` 分支同上裁掉。
 */
...
/**
 * Google-dialect error handler (`/v1beta`). Shapes built-in failures into the
 * Google envelope; delegates provider/runtime errors to `transformGoogleError`.
 * fork 缝：`DataApiError` 分支同上裁掉。
 */
...
/**
 * Cherry REST error handler — for Cherry's own endpoints (`models`) and the
 * app-level fallback (`/health`, `/`, unmatched routes). Speaks the
 * `{ error: { code, message, details? } }` envelope vocabulary, so there is no
 * provider delegate. fork 缝：`DataApiError` 分支随 DataApi 一并裁掉。
 */
