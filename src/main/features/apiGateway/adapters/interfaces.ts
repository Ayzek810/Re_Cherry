...
/**
 * fork 缝：V2 的 `@shared/data/types/{message,model,provider}` 依赖
 * `@cherrystudio/provider-registry`（workspace 包，fork 不移植），此处按
 * adapters 实际消费面定义最小结构类型，字段名与 V2 一致：
 * - `CherryUIMessage`：V2 为 `UIMessage<CherryUIMessageMetadata, CherryDataPartTypes>`；
 *   adapters 只构造/遍历 AI SDK 标准部件（text / file / reasoning / dynamic-tool，
 *   见四个 MessageConverter 与 utils/agentContinuation），从不读写
 *   `message.metadata` 与任何 `data-*` 部件（grep 成员访问证实），故取 ai 包
 *   默认泛型 `UIMessage`（unknown metadata + `UIDataTypes`）。
 * - `Model` / `Provider`：adapters 对二者零成员访问——仅在
 *   `IMessageConverter.extractProviderOptions` 签名位透传给
 *   `converters/providerOptionsMapper`（fork 下为 passthrough 缝，见该文件），
 *   故只保留 V2 同名标识字段，行为面字段全部省略。
 */

/** Cherry Studio's UIMessage with custom metadata and data part types. */
export type CherryUIMessage = UIMessage

/**
 * fork 缝：V2 `Model`（zod `ModelSchema` 推导，447 行）的最小结构替身。
 * 标识字段取自 V2 `ModelSchema` 必有项；无成员访问点，仅签名位透传。
 */
...
/**
 * fork 缝：V2 `Provider`（zod `ProviderSchema` 推导，316 行）的最小结构替身。
 * 标识字段取自 V2 `ProviderSchema` 必有项；无成员访问点，仅签名位透传。
 */
