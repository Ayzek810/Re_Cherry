 ...
 * fork 缝：语言集收敛到 fork 的 LanguageVarious（zh-CN/en-US）；未覆盖的 locale
 * 回退 Scalar 自带英文 chrome（V2 对 zh-TW 等的同款策略）。
 */
...
/**
 * One literal `t` call per slot（fork 缝：t 为 ./docsI18n 的本地键表查询，V2 的
 * scripts/i18n-check.ts 静态闸未随 fork 移植）. The return type ties the two
